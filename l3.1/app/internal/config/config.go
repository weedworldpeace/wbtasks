...
type Config struct {
	...
	EmailSenderConfig transport.EmailSenderConfig
...
func (c *Config) valid() error {
	...
	if err := validPort(c.EmailSenderConfig.Port); err != nil {
		resultErr = multierr.Append(resultErr, err)
	}
	if err := validPort(c.RabbitConfig.Port); err != nil {
		resultErr = multierr.Append(resultErr, err)
	}
...
func validReleaseMode(mode string) error {
	if mode != "" && mode != "release" {
		return models.ErrBadReleaseMod
	}
	return nil
}
